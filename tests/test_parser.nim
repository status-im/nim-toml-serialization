...
proc testNumDateToml() =
  suite "test num or date parse to toml":
    test "parseNumOrDate toml":
      ...
      x = TomlValueRef(kind: TomlKind.Int, intVal: 0)
      testNumOrDate("0", x)
      testNumOrDate("-0", x)
      testNumOrDate("+0", x)
      testNumOrDate("0 ", x)
      ...
      d = TomlDate(year:1970, month:8, day:8)
      x.dateTime.date = some(d)
      ...
      x = TomlValueRef(kind: TomlKind.Float, floatVal: 123E1'f64)
